...
class slider extends admin {
  ...
  public function post(){
    ...
    if (isset($_FILES["logo"])) {
      if ($_FILES["logo"] != "") {
        $upload_logo = image_upload(array($_FILES["logo"]), $slider_dir . "/");
  ...
  public function update(){
    ...
    if (isset($_FILES["logo_new"])) {
      if (!empty($_FILES["logo_new"]["name"])) {
        $upload_logo = image_upload(array($_FILES["logo_new"]), "assets/images/backend/slider/" . $id . "/");
        if ($upload_logo->response == OK_STATUS) {
          $image_logo_name = $upload_logo->data[0];
          if ($old_logo != "") {
            $remove_old = unlink('./assets/images/backend/slider/' . $id . '/' . $old_logo);
          }
        } else {
